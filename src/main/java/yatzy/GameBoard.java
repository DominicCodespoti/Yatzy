...
public class GameBoard {
  ...
  private static GameBoardOption[] removeOptionFromGameBoardAtIndex(GameBoardOption[] originalArray,
      int index) {
    if (originalArray == null || index < 0 || index >= originalArray.length) {
      return originalArray;
    }
    GameBoardOption[] copyOfOriginalArrayWithOptionRemoved = new GameBoardOption[
        originalArray.length - 1];
    for (int arrayIterator = 0, arrayIteratorPlusOne = 0; arrayIterator < originalArray.length;
        arrayIterator++) {
  ...
  public GameBoardOption getSpecificOptionOnBoard(String userInput) {
    for (int boardOptionIterator = 0; boardOptionIterator < optionsOnGameBoard.length;
        boardOptionIterator++) {
      if (optionsOnGameBoard[boardOptionIterator].optionName().toLowerCase()
          .equals(userInput.toLowerCase())) {
        GameBoardOption copyOfSelectedOption = optionsOnGameBoard[boardOptionIterator];
        optionsOnGameBoard = removeOptionFromGameBoardAtIndex(optionsOnGameBoard,
            boardOptionIterator);
  ...
  public String getHighestScoringOptionID(Dice givenDice) {
    ...
    for (int boardOptionIterator = 0; boardOptionIterator < optionsOnGameBoard.length;
        boardOptionIterator++) {
      for (int boardOptionIteratorAhead = 0; boardOptionIteratorAhead < optionsOnGameBoard.length;
          boardOptionIteratorAhead++) {
        if (optionsOnGameBoard[boardOptionIteratorAhead].calculateScoreFromGivenDice(givenDice)
            > highestScoringOptionValue) {
          highestScoringOptionIndex = boardOptionIteratorAhead;
          highestScoringOptionValue = optionsOnGameBoard[boardOptionIteratorAhead]
              .calculateScoreFromGivenDice(givenDice);
        }
      }
    }
  ...
  public GameBoardOption[] getOptionsOnGameBoard() {
    return optionsOnGameBoard.clone();
  }
}
