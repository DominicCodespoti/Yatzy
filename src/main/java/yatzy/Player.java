...
public class Player {
  ...
  public int selectGameBoardOption(String selectedOption, Dice dice) {
    return personalGameBoard.getSpecificOptionOnBoard(selectedOption)
        .calculateScoreFromGivenDice(dice);
  }
  ...
  public Dice rollDice() {
    ...
    while (Integer.parseInt(playerInput) != 0 && rerollCount != 3) {
      ...
      System.out.println(getPlayerName()
          + ", are you happy with these values (0), or would you like to reroll (1): ");

      if (!getIsPlayerRobot()) {
        playerInput = String
            .valueOf(InputHandler.validateAndReturnUserInputForChoices(System.in, 1));
      } else {
        playerInput = "0";
      }
      ...
      while (Integer.parseInt(playerInput) == 1 && rerollCount < MAX_REROLL_COUNT) {
        ...
        System.out.println(
            "Please enter the numbers you would like to keep (Example: 2,3,1): "); //TODO: print dice
        int[] numbersToKeepWhenRerolling = InputHandler
            .validateAndReturnUserInputForDiceReroll(System.in);
        ...
        while (!invalidReroll) { //TODO: Fix bug where correct reroll fails
          ...
          for (int i : numbersToKeepWhenRerolling) {
            for (int diceIterator2 = 0; diceIterator2 < gameDice.getAllDiceValues().length;
                diceIterator2++) {
              if (i == gameDice.getAllDiceValues()[diceIterator2]
                  && amountOfDiceToRerollMatchGameDice < numbersToKeepWhenRerolling.length) {
          ...
          if (amountOfDiceToRerollMatchGameDice != numbersToKeepWhenRerolling.length) {
            System.out.println(
                "Error: The dice you attempted to reroll do not match the below dice, try again: ");
            OutputHandler.printAllDiceRollValues(gameDice);
            numbersToKeepWhenRerolling = InputHandler
                .validateAndReturnUserInputForDiceReroll(System.in);
          } else {
            gameDice = new Dice(
                Arrays.stream(numbersToKeepWhenRerolling).boxed().collect(Collectors.toList()));
            OutputHandler.printAllDiceRollValues(gameDice);
            invalidReroll = true;
          }
        }
        rerollCount++;
        System.out.println("You have " + (MAX_REROLL_COUNT - rerollCount)
            + " rerolls left. Are you happy with these values (0), or would you like to reroll (1): ");
        playerInput = String
            .valueOf(InputHandler.validateAndReturnUserInputForChoices(System.in, 1));
  ...
  public void selectGameOption(Dice gameDice) {
    System.out.println(
        "Here are the possible options on your scoreboard, please enter the name of the option you would like to pick: ");
    String playerInput;
    OutputHandler.drawGameBoard(gameDice, getPersonalGameBoard());
    if (!getIsPlayerRobot()) {
      playerInput = InputHandler
          .validateAndReturnUserInputForGameBoardOptions(getPersonalGameBoard(), System.in);
    } else {
  ...
  private void printCurrentPlayerScore(Player listOfPlayer) {
    System.out.println(
        listOfPlayer.getPlayerName() + ", your score is now:  " + listOfPlayer.getPlayerScore());
  }
}
