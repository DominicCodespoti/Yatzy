...
public final class InputHandler {
  ...
  public static String validateAndReturnUserInputForGameBoardOptions(GameBoard gameBoardReference,
  ...
  public static int[] validateAndReturnUserInputForDiceReroll(InputStream inputStream) {
    ...
    if (userInput.matches("^(\\d(,?)){1,5}")) {
      ...
      System.out.println(
          "Error: Input does not match pattern of 'number' (5) or 'number comma number repeat' (5,4) pattern, try again:");
      return validateAndReturnUserInputForDiceReroll(inputStream);
    }
  }

  public static int validateAndReturnUserInputForChoices(InputStream inputStream,
      Integer boundaryInput) {
    Scanner initialInputReader = new Scanner(inputStream);
    String userInput = initialInputReader.nextLine();
